...
public class Indexer
{
    ...
    private void indexDocuments() throws IOException {
        ...
        ArrayList<Document> documents = getHTMLDocuments(); // DONE
    ...
    private ArrayList<Document> getHTMLDocuments()
    {
        ...
        if (files != null)
        {
            ...
            for (int id = 0; id < files.length; id++)
            {
                ...
                htmls.add(getHTMLDocument("pages/" + files[id].getName(), id)); // DONE
            }
            return htmls;
        }
        return null;
    ...
    private Document getHTMLDocument(String path, int id)
    {
        ...
        */
        ...
        // ----------------------------------
        Field idField = new StoredField(Constants.id, String.valueOf(id)); // DONE
        // ----------------------------------
        ...
        Field contentField = new TextField(Constants.content, content, Field.Store.NO); // DONE
        // ----------------------------------

        // TODO create a field that is stored and indexed
        // and contains file name
        // ----------------------------------
       Field nameField = new TextField(Constants.filename, file.getName(), Field.Store.YES); // DONE
        // ----------------------------------

        // TODO create an INT field (IntPoint) that is indexed
        // and contains file size (bytes, .length())
        // ----------------------------------
        Field sizeField = new IntPoint(Constants.filesize, (int) file.length()); // DONE ?
        // ----------------------------------
        // //TODO IntPoint is not stored but we want to a file size
        // ... so add another field (StoredField) that stores file size
        // ----------------------------------
        Field storedSizeField = new StoredField(Constants.filesize, file.length()); // DONE
        // ----------------------------------
        ...
        document.add(storedSizeField); // DONE
        // ----------------------------------
