...
class LIFO_Policy:
    ...
    def getURL(self, c, iteration):
        if len(self.queue) == 0:
            return "http://www.cs.put.poznan.pl/mtomczyk/ir/lab1/" + c.example + "/s0.html"
        else:
...
def getNormalisedURLs(retrievedURLs):
    normalised = [url.lower() for url in retrievedURLs]
    return normalised
